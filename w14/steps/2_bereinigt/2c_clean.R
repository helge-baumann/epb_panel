...
for(b in names(dat)) {
  ...
  if(length(attributes(dat[[b]])[["labels"]]) > 0) {
    ...
    # Label größer null bei numerischen Variablen 
    a_temp <- attributes(dat[[b]])$labels[attributes(dat[[b]])$labels >= 0]
    if(
      length(a_temp) == 2 & 
      length(unique(dat[[b]])) > 7
      ) {
...
# Check der IDs:
# dat %>% 
#   select(starts_with("lfdn"), starts_with("ID")) %>% 
#   write.csv2("output/id.csv", row.names=F, fileEncoding = "CP1252")
