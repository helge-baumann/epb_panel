...
names(companion)[3:ncol(companion)] <- paste0("W", 1:12)
...
for(b in names(dat_long)[!names(dat_long) %in% companion$Name]) {
  ...
  companion <-
    rbind(companion,
          c(b, attributes(dat_long[[b]])$label, rep("", 12), "generiert/zugespielt"))
  
}
  
saveWidget(datatable(companion), "output/companion_w12.html")
